...
We experimented with auto-sklearn by OFAT
...
Username of niet => vertrouwen van mensen

Subreddits meer vertrouwen?
...
We decided to make the predictions with the actual test data. After submitting we received a score of 64\%.

To investigate this drop in succesfulness we made the confusion matrix. The predictions had a high true negative rate (99\%), but a fairly low true positive rate (38\%). In other words, our model is very good at saying ``no''. It is, in fact, a bit ``too good'' at saying ``no''. This explains the score of 64\%, which is based on the Receiver Operating Curve (created by plotting the TPR against the FPR).
...
Volledige trainingset nemen wanneer goede features
